...
IF @isAOAG = 'Y'
BEGIN
    IF EXISTS (SELECT * FROM sys.dm_hadr_availability_replica_states WHERE is_local = 1 AND role_desc = 'PRIMARY')
    BEGIN
        SET @role_desc = 'Primary'
    END
    ELSE IF EXISTS (SELECT * FROM sys.dm_hadr_availability_replica_states WHERE is_local = 1 AND role_desc = 'SECONDARY')
    BEGIN
        IF EXISTS (SELECT * FROM sys.availability_replicas ar
                   INNER JOIN sys.dm_hadr_availability_replica_states ars ON ar.replica_id = ars.replica_id
                   WHERE ars.is_local = 1 AND ar.secondary_role_allow_connections_desc IN ('READ_ONLY', 'ALL'))
        BEGIN
